  ...
  return (
    <nav
      className={`sticky top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-[#173B5E] backdrop-blur-md shadow-lg"
          : "bg-[#173B5E]/95 backdrop-blur-sm"
      }`}
    >
      <div className="max-w-7xl mx-auto px-5 md:px-6 flex items-center justify-between h-16">
        {/* Logo + Name */}
        <a href="#" className="flex items-center gap-2.5 shrink-0 group">
          <Image
            src="/logo.png"
            alt="Dr. André Daniel — Implantes Dentários"
            width={40}
            height={40}
          ...
          <div className="flex flex-col leading-tight">
            <span
              className="text-white font-bold text-sm md:text-[15px] group-hover:text-[#C49A15] transition-colors"
              style={{ fontFamily: "var(--font-playfair), serif" }}
            >
              Dr. André Daniel
            </span>
            <span className="text-white/50 text-[10px] md:text-[11px] tracking-wider uppercase">
              Implantodontia · RJ
            </span>
          </div>
        </a>
        ...
        <div className="hidden lg:flex items-center gap-7">
          ...
          <Link
            href="/implantes"
            className="text-[#C49A15] hover:text-white text-sm font-semibold transition-colors border border-[#C49A15]/40 hover:border-[#C49A15] rounded-full px-4 py-1.5"
          >
            Implantes
          </Link>
          <Link
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="lg:hidden bg-[#173B5E] border-t border-white/10 px-6 pb-4">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
          ...
          <Link
            href="/implantes"
            onClick={() => setMenuOpen(false)}
            className="block py-3 text-[#C49A15] font-semibold text-sm border-b border-white/10"
          >
            Implantes →
          </Link>
          <Link
