...
export default function InstitucionalHero() {
  return (
    <section className="relative bg-[#173B5E] overflow-hidden flex flex-col justify-center pt-8 pb-16 md:pt-16 md:pb-20">
      {/* Background gradient */}
      ...
      <div className="relative max-w-7xl mx-auto px-6 w-full">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <h1
              className="font-display text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold text-white leading-[1.05] mb-2 md:mb-3"
            ...
            <p
              className="font-display text-xl md:text-2xl lg:text-3xl text-[#C49A15] italic mb-4 md:mb-6"
              style={{ fontFamily: "var(--font-playfair), serif" }}
            >
              Implantes, reabilitação e estética digital.
            </p>

            <p className="text-white/75 text-sm md:text-lg leading-relaxed mb-6 md:mb-8 max-w-lg">
              <strong className="text-white font-semibold">Membro do ITI</strong> — International Team for Implantology.
              Graduado pela UFRJ, especialista pela UVA.{" "}
              <span className="hidden md:inline">Tomografia, escaneamento 3D e cirurgia guiada. Só atendimento particular.</span>
              <span className="md:hidden">Cirurgia guiada e planejamento 3D.</span>
            </p>

            {/* CTAs */}
            <div className="flex flex-wrap gap-3 md:gap-4 mb-7 md:mb-10">
              <a
                href={WA_LINK}
                target="_blank"
              ...
              <Link
                href="/implantes"
                className="flex items-center gap-2 border border-[#C49A15]/50 hover:border-[#C49A15] text-[#C49A15] hover:text-white font-medium px-6 py-3.5 md:px-7 md:py-4 rounded-full transition-all hover:bg-[#C49A15]/10 text-sm md:text-base"
              >
                Quero implante
                <span>→</span>
              </Link>
            ...
            {/* Stats */}
            <div className="flex flex-wrap gap-x-5 gap-y-3 md:gap-6 pt-5 md:pt-6 border-t border-white/10">
              {stats.map((s) => (
                <div key={s.value}>
                  <div
                  ...
                  <div className="text-white/50 text-[10px] md:text-xs mt-0.5">{s.label}</div>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            <div className="relative w-[420px] h-[520px] rounded-3xl overflow-hidden shadow-2xl">
              <Image
                src="/dr-andre.jpg"
                alt="Dr. André Daniel — Implantodontista e Odontologia Digital | Barra da Tijuca, RJ"
                fill
